...
export class ImContainerApi<TContainerApi extends ContainerApi> {
  ...
  private readonly __blades: {
    [key: string]: Tweakpane.BladeApi<BladeController<View>>;
  };
  ...
  public folder(
    title: string,
    params?: Partial<Tweakpane.FolderParams>,
  ): ImFolderApi {
    ...
    if (!ImContainerApi.imFolderApiCtor) {
      throw new Error(
        process.env.NODE_ENV === "development"
          ? "ImContainerApi.imFolderApiCtor is not set"
          : undefined,
      );
    }
  ...
  public tab(id: string, params?: Tweakpane.TabParams): ImTabApi {
    const path = id.split("/");
    if (path.length > 1) {
      const folder = path[0];
      const remainingPath = path.slice(1).join("/");
    ...
    if (!ImContainerApi.imTabApiCtor) {
      throw new Error(
        process.env.NODE_ENV === "development"
          ? "ImContainerApi.imTabApiCtor is not set"
          : undefined,
      );
    }
  ...
  public binding<T extends any>(
    ...
    if (init != null) {
      if (this.__params[key] == null) {
        ...
        const binding = this.containerApi.addBinding(
          this.__params,
          key,
          params,
        );
